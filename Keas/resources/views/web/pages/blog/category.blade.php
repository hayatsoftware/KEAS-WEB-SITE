...
@section('content')
    ...
    <section class="section">
        ...
        <article class="article">
            <div class="container-fluid">
                <div class="content">
                    <h1 style="display: none">{!! $category->detail->name !!}</h1>
                    @if($slider_pages)
                    ...
                    <div class="link">
                        ...
                        <a href="{!! getUrlBySitemapId(BLOG_ST_ID) !!}">{!! LangPart('all', 'TÜMÜ') !!}</a>
                        @foreach($categories as $cat)
                            ...
                            @endphp
                            @if($count > 0)
                            <a class="{!! $category->id == $cat->id ? 'active':'' !!}" href="{!! $cat->detail->url !!}">{!! $cat->detail->name !!}</a>
                            @endif
                        @endforeach
                    </div>
                    <div class="row pb-5 trend_list">
                        @foreach($pages as $page)
                            <div class="col-md-4 col-lg-3 col-md-6">
                                <div class="item">
                                    <a href="{!! $page->detail->url !!}">
                                        <picture><img data-src="{!! image($page->f_) !!}" class="lazy" alt="{!! $page->detail->name !!}"></picture>
