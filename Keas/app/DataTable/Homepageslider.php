...
class Homepageslider implements DataTableInterface
{
    ...
    public function columns(Builder $builder)
    {
        ...
        return $builder->parameters([
            ...
            [
                self::DATA => 'image',
                self::NAME => 'image',
                self::TITLE => 'Slider Image/Video',
                self::FOOTER => 'Slider Image/Video'
            ],
            [
                self::DATA => 'cint_1',
                self::NAME => 'cint_1',
                self::TITLE => 'Slider Type',
                self::FOOTER => 'Slider Type'
            ]
    ...
    public function ajax($website_id, Sitemap $sitemap, $parameter = null)
    {
        ...
        $pages = Page::select("pages.id", "pages.order", "pages.status", "pages.password", "pages.allowed_role_id",
            self::CREATED_AT, self::UPDATED_AT, "pages.cint_1","pages.cvar_1")
        ...
        return Datatables::eloquent($pages)
            ...
            ->editColumn('image', function ($page) {
                if( $page->cint_1 == 1 ){
                    return '<iframe src="'.$page->cvar_1.'" class="active" style="border:none;"></iframe>';
                }else{
                    return '<img src="'.image($page->detail->f_ ?? $page->f_).'"  width="100"/>';
                }
            })
            ->editColumn('cint_1', function ($page) {
                return $page->cint_1 == 1 ? "Video":"Image";
            })
