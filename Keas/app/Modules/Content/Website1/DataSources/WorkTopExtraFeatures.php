...
class WorkTopExtraFeatures extends DataSource
{
    public function getData()
    {
        return Cache::rememberForever(hash('md5', 'MediapressWorkTopExtraFeatures' ), function () {
            $data = [];
            $pages = \DB::table('pages')
                ...
                ->join('page_details', function($join){
                    $join->on('page_details.page_id', '=', 'pages.id')
                        ->where(function($query){
                            return $query->where('page_details.name', '<>', '')
                ...
                ->where('pages.sitemap_id', WORKTOP_EXTRA_FEATURES_ST_ID)
