...
class exportProducts extends Command
{
    ...
    {
        $category_data = [];
        $categories = Category::whereIn('category_id', [5])->where('status', 1)->get();
        foreach($categories as $category){
            $category_data[] = $category->detail->name.'|'.$category->id;
        }
        $category_id = $this->choice('Please select category', $category_data, 0);
        $categoryExplode = explode('|', $category_id);
        $selectedCategories = Category::where('category_id', $categoryExplode[1])->where('status', 1)->get()->pluck('id')->toArray();
        $products = Page::where('status', 1)
            ->where('sitemap_id', PRODUCT_ST_ID)
            ->whereHas('categories', function($query) use ($selectedCategories){
                return  $query->whereIn('id', $selectedCategories);
            })
            ->get();
        $i = -1;
        $export_data = [];
                ...
                if($decor[1] != ""){
                    ...
                    foreach( $decorData as $decor ){
                        $decorPage = \DB::table('pages')
                            ->select('pages.id', 'pages.sitemap_id', 'page_details.name')
                            ->where('sitemap_id', DECORATIVE_PANEL_DECORS_ST_ID)
                ...
                if( $extraFeatures[1] != "" ){
                    ...
                    foreach( $featuresData as $feature ){
                        $featurePage = \DB::table('pages')
                            ->select('pages.id', 'pages.sitemap_id', 'page_details.name')
                            ->where('sitemap_id', DECORATIVE_PANEL_EXTRA_FEATURES_ST_ID)
                ...
                if( $surfaces[1] != "" ){
                    ...
                    foreach( $surfacesData as $surface ){
                        $surfacePage = \DB::table('pages')
                            ->select('pages.id', 'pages.sitemap_id', 'page_details.name')
                            ->where('sitemap_id', DECORATIVE_PANEL_SURFACE_ST_ID)
        ...
        dd(Storage::disk('public')->put('decorative_panel_products.json', json_encode($result_array, JSON_UNESCAPED_UNICODE)));
    }
}
